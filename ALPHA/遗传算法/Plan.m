...
tic;                                           %计时开始
%% 加载地图
dem=load('alphaMap.mat');   %截取是台湾地区数据 900平方公里
map.Z=dem.map.Z;
map.X=dem.map.X;
map.Y=dem.map.Y;
map.gap=30;                            %相邻间隔
...
%% 初始点部分
Departure=[1 1];
Destination=[100 100];                       %这两个是接口,此处的坐标为在30m每格高程图下的坐标，非实际的1m的坐标
start_x=Departure(1);
start_y=Departure(2);
end_x=Destination(1);
end_y=Destination(2);
...
%     x0=rand(1);                              %随机范围0~0.99避免取值0，0.25，0.5，0.75，1.0导致混沌失效
    ...
    for j=2:1:pathnum-1               %出发点和目的点不动
        tmpGap=floor((path(i,pathnum,1)-path(i,1,1))/(pathnum-1));
        path(i,j,1)=path(i,1,1)+(j-1)*tmpGap;     %固定x轴的间距
        y1=4*y0*(1-y0);
