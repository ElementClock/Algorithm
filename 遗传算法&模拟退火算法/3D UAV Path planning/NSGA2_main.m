...
clc;
close all;
tic           %简单地说、tic和toc是用来记录matlab命令执行的时间，tic用来保存当前时间，而后使用toc来记录程序完成时间。
...
hmax=max(max(DEM.Z));
hmin=min(min(DEM.Z));                         %DEM.Z=randint(size(DEM.Z,1),size(DEM.Z,2),[0 100]);
ganum=100;                                             %遗传代数
dnanum=50;                                             %DNA数量，
dnalength=50;                                          %DNA长度，路径点数
childrennum=dnanum;                            %子代种群
...
dna1=zeros(dnanum,dnalength+1,3);                                            %第一条路的基因
dna2=zeros(dnanum,dnalength+1,3);                                            %第二条路的基因
dna3=zeros(dnanum,dnalength+1,3);                                            %第三条路的基因
totalfitness=zeros(2,ganum+1,3);
%make a 3*101*3 size matrix ,the last 3 means three dimensional
...
for n=1:1:ganum
    disp(n);                                                                        %循环输出现在是迭代到第几代基因
    
    fitness1=NSGA2_fitness(dna1);                                 %计算基因的适应度的前置
    ...
    childrenfitness1=NSGA2_fitness(children1);                                              %子种群适应度
    childrenfitness2=NSGA2_fitness(children2);
    childrenfitness3=NSGA2_fitness(children3);
    
    combinefitness1(1:1:dnanum,:)=fitness1(1:1:dnanum,:);                           %合并适应度
...
bestdna1=zeros(bestnum,dnalength+1,3);                                               %提取最优基因
